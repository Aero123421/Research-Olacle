...
from codex_research_harness.loop import (
    inspect_research_loop,
    render_loop_instruction,
    write_loop_state,
)
...
class ResearchLoopTests(unittest.TestCase):
    def test_state_machine_derives_planner_executor_handoff_transitions(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            transition_research_plan(paths, "RP-001", status="researching")
            transition_research_plan(paths, "RP-001", status="ready")
            ...
            claimed = claim_executor(paths, "C-001", session_id="goal-session")
    ...
    def test_expired_executor_lease_requires_audited_recovery(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            paths = make_repo(Path(raw))
            ...
            activate_campaign(paths, "C-001")
            build_executor_context(paths, "C-001")
            claim_executor(paths, "C-001", session_id="expired-goal")

            state_path = paths.campaigns / "C-001" / "STATE.json"
            state = read_json(state_path)
            state["executor"]["lease_expires_at"] = "not-a-timestamp"
            atomic_write_json(state_path, state)

            decision = inspect_research_loop(paths)
            self.assertEqual(decision.action, "recover_executor")
            instruction = render_loop_instruction(decision)
            self.assertIn("--force-stale-claim", instruction)
            self.assertIn("--take-over-stale", instruction)
            self.assertIn("external compute stopped", instruction)
